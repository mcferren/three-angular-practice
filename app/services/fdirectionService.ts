...
export class FDirectionService {

    private graphstore: any;
    private scene: any;

    public init(container: HTMLElement, emitter: any) {
    ...
    public rollWhiteList( label ) {

        let cur = [];

        _self.graphstore.nodes.map(outer_element => {
            ...
            // if( parseInt( outer_element.id.indexOf( label )) < 0 ) {
                ...
                outer_element.nodesTo.map(inner_element => {
        ...
        while(nodes.length != 0 && steps < that.nodes_count) {
            ...
            for(var i=1; i <= numEdges; i++) {
                ...
                if(_self.graphstore.addNode(target_node)) {
                    ...
                    let edge_index =  _self.graphstore.addEdge(node, target_node));
